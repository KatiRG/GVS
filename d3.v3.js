d3 = function() {
  ...
  var pi2 = Math.PI, epsilon = 1e-6, epsilon2 = epsilon * epsilon, d3_radians = pi2 / 180, d3_degrees = 180 / pi2;
  function d3_sgn(x) {
  ...
  function d3_asin(x) {
    return x > 1 ? pi2 / 2 : x < -1 ? -pi2 / 2 : Math.asin(x);
  }
  function d3_sinh(x) {
  ...
  var d3_geo_area = {
    ...
    polygonEnd: function() {
      ...
      d3_geo_areaSum += area < 0 ? 4 * pi2 + area : area;
  ...
  function d3_geo_areaRingStart() {
    ...
    d3_geo_area.point = function(lambda, phi) {
      ...
      lambda0 = (lambda00 = lambda) * d3_radians, cosphi0 = Math.cos(phi = (phi00 = phi) * d3_radians / 2 + pi2 / 4), 
      sinphi0 = Math.sin(phi);
    };
    function nextPoint(lambda, phi) {
      lambda *= d3_radians;
      phi = phi * d3_radians / 2 + pi2 / 4;
  ...
  d3.geo.bounds = function() {
    ...
    function linePoint(lambda, phi) {
      ...
      if (p0) {
        ...
        var dlambda = lambda - lambda_, s = dlambda > 0 ? 1 : -1, lambdai = inflection[0] * d3_degrees * s, antimeridian = Math.abs(dlambda) > 180;
    ...
    function ringPoint(lambda, phi) {
      if (p0) {
        ...
        dlambdaSum += Math.abs(dlambda) > 180 ? dlambda + (dlambda > 0 ? 360 : -360) : dlambda;
    ...
    function ringEnd() {
      ...
      d3_geo_area.lineEnd();
      if (Math.abs(dlambdaSum) > epsilon) lambda0 = -(lambda1 = 180);
      range[0] = lambda0, range[1] = lambda1;
      p0 = null;
    }
  ...
  function d3_geo_clipSort(a, b) {
    return ((a = a.point)[0] < 0 ? a[1] - pi2 / 2 - epsilon : pi2 / 2 - a[1]) - ((b = b.point)[0] < 0 ? b[1] - pi2 / 2 - epsilon : pi2 / 2 - b[1]);
  }
  function d3_geo_pointInPolygon(point, polygon) {
    ...
    for (var i = 0, n = polygon.length; i < n; ++i) {
      ...
      if (!m) continue;
      var point0 = ring[0], lambda0 = point0[0], phi0 = point0[1] / 2 + pi2 / 4, sinphi0 = Math.sin(phi0), cosphi0 = Math.cos(phi0), j = 1;
      while (true) {
        if (j === m) j = 0;
        point = ring[j];
        var lambda = point[0], phi = point[1] / 2 + pi2 / 4, sinphi = Math.sin(phi), cosphi = Math.cos(phi), dlambda = lambda - lambda0, antimeridian = Math.abs(dlambda) > pi2, k = sinphi0 * sinphi;
        d3_geo_areaRingSum.add(Math.atan2(k * Math.sin(dlambda), cosphi0 * cosphi + k * Math.cos(dlambda)));
        if (Math.abs(phi) < epsilon) southPole = true;
        polarAngle += antimeridian ? dlambda + (dlambda >= 0 ? 2 : -2) * pi2 : dlambda;
      ...
      if (Math.abs(polarAngle) > epsilon) polar = true;
    }
    return (!southPole && !polar && d3_geo_areaRingSum < 0 || polarAngle < -epsilon) ^ winding & 1;
  }
  ...
  function d3_geo_clipAntimeridianLine(listener) {
    ...
    return {
      ...
      point: function(lambda1, phi1) {
        var slambda1 = lambda1 > 0 ? pi2 : -pi2, dlambda = Math.abs(lambda1 - lambda0);
        if (Math.abs(dlambda - pi2) < epsilon) {
          listener.point(lambda0, phi0 = (phi0 + phi1) / 2 > 0 ? pi2 / 2 : -pi2 / 2);
          ...
          clean = 0;
        } else if (slambda0 !== slambda1 && dlambda >= pi2) {
  ...
  function d3_geo_clipAntimeridianInterpolate(from, to, direction, listener) {
    ...
    if (from == null) {
      phi = direction * pi2 / 2;
      listener.point(-pi2, phi);
      listener.point(0, phi);
      listener.point(pi2, phi);
      listener.point(pi2, 0);
      listener.point(pi2, -phi);
      listener.point(0, -phi);
      listener.point(-pi2, -phi);
      listener.point(-pi2, 0);
  ...
  var d3_geo_clipAntimeridianPoint = [ -pi2, 0 ];
  function d3_geo_clipAntimeridianPolygonContains(polygon) {
    return d3_geo_pointInPolygon(d3_geo_clipAntimeridianPoint, polygon);
  }
  function d3_geo_clipCircle(radius) {
    var cr = Math.cos(radius), smallRadius = cr > 0, point = [ radius, 0 ], notHemisphere = Math.abs(cr) > epsilon, interpolate = d3_geo_circleInterpolate(radius, 6 * d3_radians);
    ...
    function intersect(a, b, two) {
      ...
      var deltalambda = lambda1 - lambda0, polar = Math.abs(deltalambda - pi2) < epsilon, meridian = polar || deltalambda < epsilon;
      if (!polar && phi1 < phi0) z = phi0, phi0 = phi1, phi1 = z;
      if (meridian ? polar ? phi0 + phi1 > 0 ^ q[1] < (Math.abs(q[0] - lambda0) < epsilon ? phi0 : phi1) : phi0 <= q[1] && q[1] <= phi1 : deltalambda > pi2 ^ (lambda0 <= q[0] && q[0] <= lambda1)) {
    ...
    function code(lambda, phi) {
      var r = smallRadius ? radius : pi2 - radius, code = 0;
  ...
  function d3_geo_clipExtent(x0, y0, x1, y1) {
    ...
    function corner(p, direction) {
      return Math.abs(p[0] - x0) < epsilon ? direction > 0 ? 0 : 3 : Math.abs(p[0] - x1) < epsilon ? direction > 0 ? 2 : 1 : Math.abs(p[1] - y0) < epsilon ? direction > 0 ? 1 : 0 : direction > 0 ? 3 : 2;
    }
    ...
    function clipLine(a, b) {
      ...
      if (Math.abs(dx) < epsilon && Math.abs(dy) < epsilon) return x0 <= a[0] && a[0] <= x1 && y0 <= a[1] && a[1] <= y1;
  ...
  function d3_geo_clipExtentT(num, denominator, t) {
    if (Math.abs(denominator) < epsilon) return num <= 0;
  ...
  function d3_geo_conic(projectAt) {
    var phi0 = 0, phi1 = pi2 / 3, m = d3_geo_projectionMutator(projectAt), p = m(phi0, phi1);
    p.parallels = function(_) {
      if (!arguments.length) return [ phi0 / pi2 * 180, phi1 / pi2 * 180 ];
      return m(phi0 = _[0] * pi2 / 180, phi1 = _[1] * pi2 / 180);
    };
    return p;
  }
  ...
  function d3_geo_pathContext(context) {
    ...
    function point(x, y) {
      context.moveTo(x, y);
      context.arc(x, y, pointRadius, 0, 2 * pi2);
    }
    function pointLineStart(x, y) {
      context.moveTo(x, y);
      stream.point = pointLine;
  ...
  function d3_geo_resample(project) {
    ...
    function resampleLineTo(x0, y0, lambda0, a0, b0, c0, x1, y1, lambda1, a1, b1, c1, depth, stream) {
      ...
      if (d2 > 4 * delta2 && depth--) {
        ...
        if (dz * dz / d2 > delta2 || Math.abs((dx * dx2 + dy * dy2) / d2 - .5) > .3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
  ...
  function d3_geo_projectionRadiansRotate(rotate, stream) {
    ...
    transform.point = function(x, y) {
      ...
      stream.point(x > pi2 ? x - 2 * pi2 : x < -pi2 ? x + 2 * pi2 : x, y[1]);
    };
    return transform;
  }
  ...
  function d3_geo_rotation(deltalambda, deltaphi, deltagamma) {
    return deltalambda ? deltaphi || deltagamma ? d3_geo_compose(d3_geo_rotationlambda(deltalambda), d3_geo_rotationphigamma(deltaphi, deltagamma)) : d3_geo_rotationlambda(deltalambda) : deltaphi || deltagamma ? d3_geo_rotationphigamma(deltaphi, deltagamma) : d3_geo_equirectangular;
  }
  function d3_geo_forwardRotationlambda(deltalambda) {
    return function(lambda, phi) {
      return lambda += deltalambda, [ lambda > pi2 ? lambda - 2 * pi2 : lambda < -pi2 ? lambda + 2 * pi2 : lambda, phi ];
    };
  }
  ...
  function d3_geo_circleInterpolate(radius, precision) {
    ...
    return function(from, to, direction, listener) {
      ...
      if (from != null) {
        ...
        if (direction > 0 ? from < to : from > to) from += direction * 2 * pi2;
      } else {
        from = radius + direction * 2 * pi2;
        to = radius - .5 * step;
      }
  ...
  function d3_geo_lengthLineStart() {
    ...
    function nextPoint(lambda, phi) {
      var sinphi = Math.sin(phi *= d3_radians), cosphi = Math.cos(phi), t = Math.abs((lambda *= d3_radians) - lambda0), cosdeltalambda = Math.cos(t);
  ...
  function d3_geo_conicEquidistant(phi0, phi1) {
    ...
    if (Math.abs(n) < epsilon) return d3_geo_equirectangular;
    function forward(lambda, phi) {
      var rho = G - phi;
  ...
  function d3_geo_mercator(lambda, phi) {
    return [ lambda, Math.log(Math.tan(pi2 / 4 + phi / 2)) ];
  }
  d3_geo_mercator.invert = function(x, y) {
    return [ x, 2 * Math.atan(Math.exp(y)) - pi2 / 2 ];
  };
  ...
  function d3_ease_elastic(a, p) {
    ...
    if (arguments.length) s = p / (2 * pi2) * Math.asin(1 / a); else a = 1, s = p / 4;
    return function(t) {
      return 1 + a * Math.pow(2, 10 * -t) * Math.sin((t - s) * 2 * pi2 / p);
    };
  }
  ...
  d3.layout.pie = function() {
    var value = Number, sort = d3_layout_pieSortByValue, startAngle = 0, endAngle = 2 * pi2;
    function pie(data) {
  ...
  d3.svg.arc = function() {
    ...
    function arc() {
      ...
      a0 = a1, a1 = da), a1 - a0), df = da < pi2 ? "0" : "1", c0 = Math.cos(a0), s0 = Math.sin(a0), c1 = Math.cos(a1), s1 = Math.sin(a1);
  ...
  var d3_svg_arcOffset = -pi2 / 2, d3_svg_arcMax = 2 * pi2 - 1e-6;
